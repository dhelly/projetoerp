...
public function init(){
	...
	if (file_exists($layout)){
		...
		$view = str_replace('{CONTEUDO}', $output, $layout);

		// Substitui o texto {title_for_layout} pelo valor de title em view.
		$view = str_replace('{TITULO_PAGINA}', $title, $view);

		// Links CSS.
		$view = str_replace('{CSS_LAYOUT}', $css, $view);

		// Links JS.
		$view = str_replace('{JS_LAYOUT}', $js, $view);
	}else{
		$view = $output;
	}
